...
Any link can be described by a partitioned sequence of even numbers called its Dowker-Thistlewhaite Code, or DT code. This is provided by the software SnapPy. In the case of a link $L$ fitting the description above, we use the following lemma:
...
The script "integral\_slopes.py", given the DT code of a link $L$ satisfying the conditions above, outputs a list of possible integral characteristic slopes for any knot $K$. It relies on writing a text file, running it on SnapPy, saving the output, and interpreting it repeatedly. There are certain system requirements explicit on the code. The script consists of the following steps:

\begin{enumerate}
	\item From the DT code, it writes the file "[INFO]K.py". This file asks for the volume of the knot $K$, for the volume of the manifold $Z = S_0^3(R)$ obtained by zero-surgery filling the 1-handle component, and for the length of the Seifert longitude in $Z$.
	...
	\item The program writes the file "[TEST]K.py". This file asks for the volume of each knot $K_{n}'$ in the twist family of $K$, whose exterior is easily obtained by doing $n$-surgery on the first component of the manifold $Z$, for all $n$ such that $|n| \leq N$.
...
\begin{proof} We produced the link $L$ aforementioned for all knots $K$ mentioned in Theorem~\ref{thm:low-crossing}. All the DT codes are included in the file "DT\_List.txt", which is correctly formatted for the script "integral\_slopes.py". Running it outputs the .py files labeled [OUT], and text that is reproduced in the file "results.txt". From reading this file, we see that:
	\begin{enumerate}[label=\normalfont \bf (\alph*)]
