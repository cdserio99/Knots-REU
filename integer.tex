...
To construct the knot $K_n'$ from $n$-surgery on $K$, we first change the framing of $K$ to $n$ and call it $B$. We then add a cancelling pair $R \cup G$ to $B$ in the exact same place as in the diagram $B\cup R \cup G$ that we would get by following the procedure illustrated near the end of section 2.2 to invoke Piccirillo's theorem. Note that by a slam dunk, we can change the framing of $B$ to 0 if we add a $-1/n$-framed meridian $P$ to $B$. Specifically when $n=0$, such a slam dunk leads to adding $P$ with $-\infty$ framing to $B$ while the framing of $B$ remains unchanged, which amounts to doing nothing to $B$. We slide $B$ over $R$ and isotope the diagram until $B$ and $R$ are both unknots that cross each other twice (i.e., $B$ and $R$ form a Hopf link if $G$ were removed). Then slide $G$ over $B$ until $G$ is no longer linked with $R$. Now we can remove $R$ using a "lightbulb trick" that would leave us a two-component link $G\cup P$, with $G$ as the knot $K_0'$ that shares the same 0-surgery as $K$ (since a slam dunk amounted to not having added any $P$ in the first place) and $P$ as an unknot $c'$ linked with $K_0'$. Observe that the framing of $P$ remains unchanged throughout this process and we can therefore do a $n$-Rolfsen twist on $G\cup P$ to get rid of $P=c'$. As a result, we are left with only one knot, the knot obtained from twisting $K_0'$ along $c'$ for $n$ times, which is exactly $K_n'$ as desired.
...
\begin{proof}
	We show that if $c'$ and $K_0'$ are surgery duals to $K$ and $c$ respectively, then $c$ and $K$ are dual to $K_0'$ and $c'$ respectively, which is essentially the symmetry of surgical duality in $S^{3}$. Suppose we perform $n$-surgery on $Q$, a knot component, then its dual $Q'$ in $S^{3}_{n}(K)$ can be represented as a meridian to it. Note that both $K_0'$ and $c$ are 0-framed in our construction and hence it suffices to show that the duality is symmetric in the case where we perform 0-surgery on $Q'$ and get back $S^{3}$ from $Q \cup Q'$. The dual to $Q'$ is a 0-framed meridian $Q''$ to it. We then slide $Q''$ over $Q$, after which $Q''$ inherits the $n$-framing from $Q$ since $lk(Q, Q'')=0$ and it had 0-framing originally. After isotopes in $S^3$, we can get $Q''$ disjoint from $Q'$, leaving $n$-framed $Q$ only linked with $0$-framed $Q'$ which we can cancel now and get the $n$-framed $Q'' = Q$ as desired. 
