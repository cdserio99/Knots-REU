...
\begin{proof}
Let $c' \cup K'$ be the surgery dual link as a result of $(0, 0)$-surgery on $K \cup c$. Then $(0,0)$-surgery on $K'\cup c'$ is $S^3$ by the remarks before Lemma 2.5, and since by assumption $c$ is not a meridian to $K$, it follows from the contrapositive of Lemma 2.5 that $c'$ is not a meridian to $K'$. Thus by Lemma 2.6, all but finitely many of the knots $K_n'$ sharing the same $n$-surgery with $K$ are not isotopic to $K$, i.e., all but finitely many integers are non-characterizing slopes for $K$.
\end{proof}
